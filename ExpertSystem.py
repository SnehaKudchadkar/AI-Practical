...
# Q6
    ...
    if answer == "yes":
        ...
        if warning_lights == "abs":
            symptoms.append("ABS warning light")
        elif warning_lights == "check engine":
...
symptoms = ask_symptoms()  # Collect symptoms from user input
issues = diagnose_issues(symptoms)  # Diagnose issues based on symptoms

# Output the results
print("\nPossible car issues based on symptoms:")
for issue in issues:
    print("-", issue)  # Print each potential issue or diagnostic recommendation
