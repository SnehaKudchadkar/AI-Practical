...
def solve_puzzle(initial_state, goal_state, max_steps=1000):
    ...
    while current_cost > 0 and steps < max_steps:
        ...
        # The min function is used to find the smallest value in a list. In this context, it will find the neighbor with the smallest heuristic value.
        
        # key Parameter
        # The key parameter in the min function specifies a function to be called on each element in the list before making comparisons. It determines the criteria for finding the minimum value.
        
        # lambda x: heuristic(x, goal_state)
        # This is a lambda function, an anonymous function defined using the lambda keyword. In this context:
        # x represents an element from the neighbors list (i.e., a possible next state of the puzzle).
        # heuristic(x, goal_state) calculates the heuristic value of the state x with respect to the goal_state.
